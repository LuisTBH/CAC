...
int main( int argc, char *argv[] )
{
    ...
    if (idproc == 0) {
	 ...
	 b[i]=(double) i;
       ...
       for (i=1; i<numprocs; i++) {
          ...
          for (j=0; j<cadaproc; j++) {
            ...
            MPI_Send(&b[k], 1, MPI_DOUBLE, i, 1, MPI_COMM_WORLD);
	 ...
	 k++;
       ...
       prod = 0;
       for (j=0; j<cadaproc; j++) {
         MPI_Recv(&datoa, 1, MPI_DOUBLE, 0, 1, MPI_COMM_WORLD, &status);
         MPI_Recv(&datob, 1, MPI_DOUBLE, 0, 1, MPI_COMM_WORLD, &status);

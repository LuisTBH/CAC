	 ...
	 b[i]=(double) i;
       ...
       for (i=1; i<numprocs; i++) {
          ...
          printf("Soy %d. Enviando a %d el dato %d\n",idproc, i, cadaproc);
            ...
            MPI_Send(&a[k], cadaproc, MPI_DOUBLE, i, 1, 
MPI_COMM_WORLD);
            MPI_Send(&b[k], cadaproc, MPI_DOUBLE, i, 1, 
MPI_COMM_WORLD);
	 ...
	 k++;
       ...
       for (j=0; j<cadaproc; j++) {
         MPI_Recv(&a, cadaproc, MPI_DOUBLE, 0, 1, MPI_COMM_WORLD, 
&status);
         MPI_Recv(&b, cadaproc, MPI_DOUBLE, 0, 1, MPI_COMM_WORLD, 
&status);

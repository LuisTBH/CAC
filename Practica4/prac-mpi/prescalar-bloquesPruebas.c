    ...
    if (idproc == 0) {
       ...
       for (i=1; i<numprocs; i++) {
          ...
          printf("Soy %d. Enviando a %d el dato %d\n",idproc, i, cadaproc);
            ...
            /* Enviamos a cada procesador los datos que le tocan en bloques de tamaño cadaproc */
       ...
       for (j=0; j<cadaproc; j++) {
         MPI_Recv(&datoa, cadaproc, MPI_DOUBLE, 0, 1, MPI_COMM_WORLD,&status);
         MPI_Recv(&datob, cadaproc, MPI_DOUBLE, 0, 1, MPI_COMM_WORLD,&status);
